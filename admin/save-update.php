...
if(empty($_FILES['new-image']['name'])){
 ...
 $error  = array();
    ...
    if(empty($error) == true){
        move_uploaded_file($file_tmp,"admin/uplode/".$file_name);
    }else{
        print_r($error);
        die();
    }
  ...
  $query = "UPDATE post set `title` = '{$_POST["post_title"]}', `description`='{$_POST["postdesc"]}', `category`= {$_POST["category"]}, `post_img`='{$file_name}' 
